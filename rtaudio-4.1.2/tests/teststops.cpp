           ...
           double /*streamTime*/, RtAudioStreamStatus status, void *mydata )
{
  ...
  for ( i=0; i<nBufferFrames; i++ ) {
    if ( data->frameCounter % data->pulseCount == 0 ) sample = 1.0;
    else sample = -1.0;
...
int main( int argc, char *argv[] )
{
  ...
  try {
    adc->openStream( &oParams, &iParams, RTAUDIO_FLOAT32, fs, &bufferFrames, &pulse, (void *)&mydata );
